...
async def list_guests(request: Request, db: Database = Depends(get_db)) -> HTMLResponse:
    rows = await db.fetch("""
        SELECT id, full_name, phone, email, loyalty_tier, total_spend
        FROM guests ORDER BY full_name
    """)
    return templates.TemplateResponse(
        "guests/list.html", {"request": request, "guests": rows}
    )


@router.get("/new", response_class=HTMLResponse)
async def new_guest_form(request: Request) -> HTMLResponse:
    return templates.TemplateResponse("guests/create.html", {"request": request})
