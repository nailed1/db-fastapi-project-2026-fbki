...
async def login(
    ...
    if tourist and _safe_verify(password, tourist["password_hash"]):
        ...
        resp.set_cookie(
            "access_token", token, httponly=True, max_age=3600 * 8, samesite="lax"
        )
...
async def register(
    ...
    passport_taken = await db.fetchval(
        "SELECT 1 FROM guests WHERE passport = $1", passport
    )
    if passport_taken:
        return err(
            "Гость с таким паспортом уже существует — войдите вместо регистрации"
        )
