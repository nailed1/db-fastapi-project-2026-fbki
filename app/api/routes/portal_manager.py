"""Manager portal — schedules, bookings, service and replenishment."""
...
async def upsert_schedule(
    ...
    return RedirectResponse(
        url=f"/portal/manager/schedule?week={work_date}", status_code=303
    )
...
async def resolve_refund(
    ...
    if action == "approve":
        ...
        await db.execute(
            """
            UPDATE bookings SET status = 'Отменено', payment_status = 'Возврат'
            WHERE id = $1
            """,
            refund["booking_id"],
        )
        await db.execute(
            """
            UPDATE refund_requests SET status = 'Одобрена',
                resolved_at = $1, manager_id = $2 WHERE id = $3
            """,
            datetime.now(),
            user.id,
            refund_id,
        ...
        await db.execute(
            """
            UPDATE refund_requests SET status = 'Отклонена',
                resolved_at = $1, manager_id = $2 WHERE id = $3
            """,
            datetime.now(),
            user.id,
